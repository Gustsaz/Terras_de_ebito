...
/* ======= Funções de moedas (NÍVEL SUPERIOR) =======
   Cole este bloco **aqui**, antes de `document.addEventListener('DOMContentLoaded'...)`
*/
function getCoinQtyFromCharLocal(charObj, tipo) {
    try {
        const arr = Array.isArray(charObj?.moedas) ? charObj.moedas : [];
        const found = arr.find(m => String(m?.tipo) === String(tipo));
        return Number(found?.quantidade ?? 0);
    } catch (e) { return 0; }
}
function getCoinQtyFromChar(tipo) { return getCoinQtyFromCharLocal(charData || {}, tipo); }

/* ================= Prompt custom (reusa classes de alert do global.js) ================ */
/**
 * Mostra um modal com input numérico e devolve string do valor ou null se cancelado.
 * Reusa .alert-backdrop / .alert-modal do seu global.css para ficar igual aos alerts.
 */
function showPromptModal(message, defaultValue = '') {
    return new Promise((resolve) => {
        ...
        // evitar injeção simples: escapando aspas no default (basta para nosso caso)
        const safeDefault = String(defaultValue).replace(/"/g, '&quot;');

        modal.innerHTML = `
      <div class="alert-header">Editar moeda</div>
      <div class="alert-body">
        <div style="margin-bottom:8px;">${message}</div>
        <input type="number" class="coin-input" value="${safeDefault}" />
      </div>
    ...
    `;
        ...
        // animação (mesma do alert)
        requestAnimationFrame(() => backdrop.classList.add('visible'));

        const input = modal.querySelector('.coin-input');

        // foco e seleção para facilitar edição
        ...
        modal.querySelector('.alert-cancel').addEventListener('click', () => cleanup(null));
        modal.querySelector('.alert-ok').addEventListener('click', () => cleanup(input.value));
...
/* ================= Editar quantia de moeda (usa prompt customizado) ================= */
async function editCoinAmount(tipo) {
    try {
        const current = getCoinQtyFromChar(tipo);
        const raw = await showPromptModal(`Quantidade de ${tipo} (atual: ${current})`, String(current));
        if (raw === null) return; // cancelou
        const n = parseInt(String(raw), 10);
        if (Number.isNaN(n) || n < 0) { alert('Valor inválido — insira um número inteiro >= 0'); return; }

        const moedas = Array.isArray(charData?.moedas) ? charData.moedas.slice() : [];
        const idx = moedas.findIndex(m => String(m?.tipo) === String(tipo));
        if (idx >= 0) moedas[idx] = Object.assign({}, moedas[idx], { quantidade: n });
        else moedas.push({ tipo: tipo, quantidade: n });

        // atualiza local
        charData = Object.assign({}, charData, { moedas: moedas });

        // salva usando a função robusta (que você acabou de colar)
        await robustSaveCharacterField('moedas', moedas);

        // atualiza UI
        renderCoins();
    } catch (err) {
        console.error('Falha ao editar/salvar moedas:', err);
        alert('Erro ao salvar moedas. Veja console.');
    }
}


function renderCoins(newChar) {
    // se vier novo char, atualiza a cópia local
    if (newChar) charData = newChar;

    const types = ['ouro', 'prata', 'bronze'];
    types.forEach(tipo => {
        const qtyEl = document.querySelector(`.coin-qty[data-tipo="${tipo}"]`);
        if (qtyEl) qtyEl.textContent = String(getCoinQtyFromChar(tipo));
    });

    // listeners idempotentes
    document.querySelectorAll('.coin').forEach(c => {
        if (c.__coinListenerAdded) return;
        c.__coinListenerAdded = true;

        c.addEventListener('click', async (e) => {
            e.stopPropagation();
            await editCoinAmount(String(c.dataset.tipo));
        });
 ...
 * - value: novo valor para esse campo (pode ser array/obj/etc)
 *
...
async function robustSaveCharacterField(field, value) {
    ...
    try {
        // Ler o documento atual do usuário
        ...
        // Garantir que temos charData e seu uid
        if (!charData || !charData.uid) {
            throw new Error('charData indefinido ou sem uid');
        }

        const charIdx = personagens.findIndex(p => String(p?.uid) === String(charData.uid));

        if (charIdx === -1) {
            // Se não existe, adiciona novo personagem baseado em charData (fallback)
            const newChar = Object.assign({}, charData, { [field]: value });
            personagens.push(newChar);
        } else {
            // Substitui só o campo no personagem existente (mantendo os outros campos)
            const updatedChar = Object.assign({}, personagens[charIdx], { [field]: value });
            personagens[charIdx] = updatedChar;
        }

        // Atualiza o documento do usuário com o array personagens modificado
        await window.updateDoc(userDocRef, { personagens: personagens });

        // Atualiza o estado local charData para refletir o que foi salvo
        const savedChar = personagens.find(p => String(p?.uid) === String(charData.uid));
        if (savedChar) charData = savedChar;

        return charData;
    } catch (err) {
        console.error('robustSaveCharacterField erro:', err);
        throw err;
    }
}
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try { renderCoins(charData); } catch (e) { console.warn('renderCoins init erro', e); }
    ...
    function computeAllowedAndRemainingFromAttributes(attrObj) {
        ...
        // conta quantos atributos têm valor negativo (cada negativo aumenta o limite em +1)
        // isso corresponde ao cálculo de "flaws" na criação de personagem
        let countNegatives = 0;
        for (const k of ATTR_KEYS_LIST) {
            if (Number(normalizedMap[k] ?? 0) < 0) countNegatives++;
        }
        const allowedCap = 7 + countNegatives;

        // soma apenas dos valores positivos (raw) - mesma lógica da criação
        let sumPositives = 0;
        for (const k of ATTR_KEYS_LIST) {
            const val = Number(normalizedMap[k] ?? 0);
            if (val > 0) sumPositives += val;
        }

        const remaining = allowedCap - sumPositives;
        return { allowedCap, sumPositives, remaining, countNegatives };
    }
    ...
    function startAttrRemainingListener() {
        ...
        function applyRemoteCharData(newChar) {
            ...
            try { renderCoins(newChar); } catch (e) { console.warn('renderCoins erro', e); }

            /* ===================== Inicialização: invocar após charData pronto ===================== */
            /* 
              1) Chame renderCoins() logo depois de carregar/exibir o personagem inicialmente.
                 (No seu fluxo atual você já tem um ponto após carregar charData — chame ali.)
              2) Dentro de applyRemoteCharData(newChar) (onde você já chama renderProficiencies(newChar)),
                 adicione também: renderCoins(newChar);
            */

            /* Exemplo de chamadas (cole se preferir perto do final da seção de inicialização): */
            try { renderCoins(); } catch (e) { /* silent */ }
        ...
        attrEl.addEventListener('click', async (e) => {
            ...
            const finish = async (commit) => {
                if (commit) {
                    ...
                    // conta quantos atributos têm valor negativo (cada negativo aumenta o limite em +1, considerando o novo valor)
                    let countNegatives = 0;
                    for (const k of ATTR_KEYS_LIST) {
                        const kn = normalize(k);
                        const val = (kn === normalizedKey) ? newValue : Number(attributes[kn] ?? 0);
                        if (val < 0) countNegatives++;
                    }
                    const allowedCap = 7 + countNegatives;

                    // soma apenas dos valores positivos com a substituição do valor em edição - mesma lógica da criação
                    let sumPositives = 0;
                    for (const k of ATTR_KEYS_LIST) {
                        const kn = normalize(k);
                        const val = (kn === normalizedKey) ? newValue : Number(attributes[kn] ?? 0);
                        ...
                        if (userDocSnap.exists()) {
                            ...
                            if (index >= 0) {
                                ...
                                (() => {
                                    ...
                                    // conta quantos atributos têm valor negativo (cada negativo aumenta o limite em +1)
                                    let countNegativesSaved = 0;
                                    for (const k of ATTR_KEYS_LIST) {
                                        if (Number(normalizedMapSaved[k] ?? 0) < 0) countNegativesSaved++;
                                    }
                                    const allowedCapSaved = 7 + countNegativesSaved;

                                    // soma apenas dos valores positivos (raw) - mesma lógica da criação
                                    let sumPositivesSaved = 0;
                                    for (const k of ATTR_KEYS_LIST) {
                                        const val = Number(normalizedMapSaved[k] ?? 0);
  ...
  `;
     ...
     */
    // Substituir função handleLevelUps antiga por esta.
    // Objetivo: apenas ajustar LVL, pontos_restantes, history (com addPV/addMN/addSTA == 0)
    // e persistir no Firestore. NÃO altera PV/MN/STA.totals nem atuais.
    async function handleLevelUps(previousLevel, newLevel) {
        if (!window.firebaseauth?.currentUser) return;
        const uid = window.firebaseauth.currentUser.uid;
        const userDocRef = window.doc(window.firestoredb, 'usuarios', uid);
        const userDocSnap = await window.getDoc(userDocRef);
        if (!userDocSnap.exists()) {
            console.warn('handleLevelUps: documento de usuário não encontrado');
            return;
        }
        const data = userDocSnap.data() || {};
        const characters = Array.isArray(data.personagens) ? data.personagens.slice() : [];
        const idx = characters.findIndex(c => c && c.uid === charUid);
        if (idx < 0) {
            console.warn('handleLevelUps: personagem não encontrado');
            return;
        }

        // Para cada nível ganho, incrementa somente pontos_restantes e registra histórico (sem alterar PV/MN/STA)
        const char = characters[idx];
        char.levelHistory = Array.isArray(char.levelHistory) ? char.levelHistory : [];

        for (let lvl = previousLevel + 1; lvl <= newLevel; lvl++) {
            // Salva estado antes (para possível reversão), mas sem tocar PV/MN/STA
            const attrsBefore = JSON.parse(JSON.stringify(char.atributos || {}));
            const prBefore = Number(char.pontos_restantes ?? 0);

            // 1) pontos_restantes +1
            char.pontos_restantes = (Number(char.pontos_restantes ?? 0) + 1);

            // 2) registra histórico com addPV/addMN/addSTA = 0 (sem alterar totals)
            const histEntry = {
                lvl: lvl,
                addPV: 0,
                addMN: 0,
                ...
                pontosRestantesBefore: prBefore,
                ts: Date.now()
            };
            char.levelHistory.push(histEntry);

            // 3) Se nível múltiplo de 5: manter modal de escolha de proficiência (se desejar)
            // (se quiser remover esse modal, comente o bloco abaixo)
            if (lvl % 5 === 0) {
                try {
                    // reutiliza o modal/procedimento que você já tem para escolher proficiências.
                    // A implementação original espera atualizar char.proeficiencias; mantemos isso.
                    // openProficiencyModal deve estar definida no seu arquivo; se não estiver, pule.
                    if (typeof openProficiencyModal === 'function') {
                        // Modal síncrono/assíncrono: se o modal retornar escolha, salva em profAdded
                        // Aqui fazemos um await para preservar ordem — se preferir fire-and-forget, remova await.
                        const chosen = await openProficiencyModal({ title: 'Escolha uma Proficiência', initialValue: '' });
                        if (chosen) {
                            char.proeficiencias = Array.isArray(char.proeficiencias) ? char.proeficiencias : [];
                            if (!char.proeficiencias.includes(chosen)) {
                                char.proeficiencias.push(chosen);
                                // grava qual prof foi adicionada no histórico
                                char.levelHistory[char.levelHistory.length - 1].profAdded = chosen;
                            }
                        }
                    }
                } catch (err) {
                    console.warn('Erro ao abrir modal de proficiencia (handleLevelUps):', err);
                }
            }
        }

        // Atualiza LVL e EXP/obj local (não mexemos em PV/MN/STA)
        char.LVL = Number(newLevel);
        // char.EXP.atual já é salvo por quem chamou handleLevelUps (gainExp/normalize), mas garantir estrutura:
        char.EXP = char.EXP || {};
        char.EXP.atual = Number(currentEXP ?? 0);

        // Persiste mudanças no Firestore
        characters[idx] = char;
        await window.updateDoc(userDocRef, { personagens: characters });

        // Atualiza charData local e UI (mantendo PV/MN/STA como estavam)
        charData = characters[idx];
        try {
            // atualiza exibição de pontos_restantes e nível
            updateRemainingUI();
        } catch (e) { /* silent */ }
        try {
            const lvlEl = document.getElementById('exp-level');
            ...
            applyBar('exp-bar-fill', 'exp-bar-text', currentEXP, expLimit, 'exp');
            // não atualizamos stat-pv/stat-mn/stat-sta porque não houve mudança de totals
        } catch (e) { /* silent */ }
    }
...
*/
    ...
    /* === ADD: permitir usuário aumentar o máximo (PV/MN/STA) via modal quando já estiver no máximo ===
   Copiar/colar este bloco logo após a função wirePlusMinus(...) já existente.
*/

    (function enableStatMaxIncreaseModal() {
        // helper: calcula os totals usando eventuais baseOverride salvos em charData
        function computeTotalsWithOverride() {
            try {
                // bases por classe (constantes originais)
                const classBases = {
                    PV: { Arcanista: 8, Escudeiro: 18, Luminar: 9, Errante: 10 },
                    MN: { Arcanista: 10, Escudeiro: 2, Luminar: 10, Errante: 5 },
                    STA: { Arcanista: 6, Escudeiro: 8, Luminar: 6, Errante: 12 }
                };

                const cls = String(savedClass || (charData && charData.classe) || '').trim();

                // escolha do atributo que afeta cada stat (PV -> bravura, MN -> arcano, STA -> folego/essencia dependendo da classe)
                const brav = Number(attrKeys?.bravura ?? 0);
                const arcn = Number(attrKeys?.arcano ?? 0);
                const foleg = Number(attrKeys?.folego ?? 0);
                const ess = Number(attrKeys?.essencia ?? 0);

                // PV
                const pvBase = (charData && charData.PV && typeof charData.PV.baseOverride !== 'undefined')
                    ? Number(charData.PV.baseOverride)
                    : (classBases.PV[cls] ?? 0);
                const pvTotal = pvBase + brav;

                // MN
                const mnBase = (charData && charData.MN && typeof charData.MN.baseOverride !== 'undefined')
                    ? Number(charData.MN.baseOverride)
                    : (classBases.MN[cls] ?? 0);
                const mnTotal = mnBase + arcn;

                // STA: atributo depende da classe (Luminar/Errante usam ESSENCIA, outros usam FOLEGO)
                const staAttr = (cls === 'Luminar' || cls === 'Errante') ? ess : foleg;
                const staBase = (charData && charData.STA && typeof charData.STA.baseOverride !== 'undefined')
                    ? Number(charData.STA.baseOverride)
                    : (classBases.STA[cls] ?? 0);
                const staTotal = staBase + staAttr;

                // atualizar locais e UI-related variables
                charData = charData || {};
                charData.PV = Object.assign({}, charData.PV || {}, { total: pvTotal });
                charData.MN = Object.assign({}, charData.MN || {}, { total: mnTotal });
                charData.STA = Object.assign({}, charData.STA || {}, { total: staTotal });

                // atualizar variáveis usadas pelo resto do script
                if (typeof totPV !== 'undefined') totPV = pvTotal;
                if (typeof totMN !== 'undefined') totMN = mnTotal;
                if (typeof totSTA !== 'undefined') totSTA = staTotal;

                // ajustar curretos para não ultrapassar os novos totals
                if (typeof curPV !== 'undefined' && curPV > pvTotal) curPV = pvTotal;
                if (typeof curMN !== 'undefined' && curMN > mnTotal) curMN = mnTotal;
                if (typeof curSTA !== 'undefined' && curSTA > staTotal) curSTA = staTotal;

                // atualiza UI
                try { updateAllBars(); } catch (e) { /* silencioso */ }

                return { pvTotal, mnTotal, staTotal };
            } catch (err) {
                console.warn('computeTotalsWithOverride erro:', err);
                return null;
            }
        }

        // chama uma vez para garantir coerência (caso já haja baseOverride salvo)
        try { computeTotalsWithOverride(); } catch (e) { }

        // mapeamento de stat -> metadados para tratar PV/MN/STA
        const STAT_META = {
            pv: { field: 'PV', attrKey: (cls => 'bravura') },
            mn: { field: 'MN', attrKey: (cls => 'arcano') },
            sta: { field: 'STA', attrKey: (cls => (cls === 'Luminar' || cls === 'Errante') ? 'essencia' : 'folego') }
        };

        // handler genérico para + clicks
        async function onPlusClickedFor(stat) {
            try {
                // pega valores atuais/total
                const meta = STAT_META[stat];
                if (!meta) return;

                const current = (stat === 'pv' ? (curPV ?? 0) : (stat === 'mn' ? (curMN ?? 0) : (curSTA ?? 0)));
                const total = (stat === 'pv' ? (totPV ?? 0) : (stat === 'mn' ? (totMN ?? 0) : (totSTA ?? 0)));

                // somente abrir modal se já está no máximo
                if (current < total) {
                    // deixar o comportamento padrão (incremento) acontecer — nada a fazer
                    return;
                }

                // solicitar novo máximo final (valor que usuário quer ver como total)
                const label = stat.toUpperCase();
                const raw = await showPromptModal(`Novo máximo para ${label} (atual: ${total})`, String(total));
                if (raw === null) return; // cancelou
                const n = parseInt(String(raw), 10);
                if (Number.isNaN(n) || n < 0) { alert('Valor inválido — insira um inteiro >= 0'); return; }

                // determinar atributo que afeta esse stat (usando savedClass)
                const cls = String(savedClass || (charData && charData.classe) || '').trim();
                const attrName = STAT_META[stat].attrKey(cls);
                const attrBonus = Number(attrKeys?.[attrName] ?? 0);

                // calculamos o "baseOverride" que, somado ao atributo, produz o número final desejado
                // baseOverride = desiredFinal - atributoAtual
                const baseOverride = Math.floor(n - attrBonus);

                // preparamos o objeto a salvar (mantendo o campo 'atual' coerente)
                const existing = charData && charData[meta.field] ? Object.assign({}, charData[meta.field]) : {};
                const newObj = Object.assign({}, existing, {
                    baseOverride: baseOverride,
                    total: baseOverride + attrBonus,
                    atual: Math.min(current, Math.max(0, baseOverride + attrBonus))
                });

                // salva usando a sua rotina robusta (substitui apenas o campo PV/MN/STA do personagem)
                await robustSaveCharacterField(meta.field, newObj);

                // atualiza local e recalcula totals com override (pra refletir alterações)
                charData = charData || {};
                charData[meta.field] = newObj;

                computeTotalsWithOverride();

                // feedback leve
                try {
                    // highlight/flash na barra do stat
                    const fillId = stat === 'pv' ? 'pv-bar-fill' : (stat === 'mn' ? 'mn-bar-fill' : 'sta-bar-fill');
                    const el = document.getElementById(fillId);
                    if (el) {
                        el.animate([{ opacity: 1 }, { opacity: 0.6 }, { opacity: 1 }], { duration: 380 });
                    }
                } catch (e) { /* silent */ }

            } catch (err) {
                console.error('Erro ao salvar novo máximo do stat:', err);
                alert('Falha ao salvar novo máximo. Veja o console.');
            }
        }

        // ---------- substituição robusta dos + buttons: clona para remover listeners antigos e registra handler único ----------
        (function replacePlusButtonsAndWireHandlers() {
            const plusButtons = [
                { id: 'pv-plus', stat: 'pv' },
                { id: 'mn-plus', stat: 'mn' },
                { id: 'sta-plus', stat: 'sta' }
            ];

            plusButtons.forEach(({ id, stat }) => {
                const orig = document.getElementById(id);
                if (!orig) return;

                // clona o nó para remover event listeners previamente anexados
                const clone = orig.cloneNode(true);
                orig.parentNode.replaceChild(clone, orig);

                // handler único que controla incremento OU abertura do modal
                clone.addEventListener('click', (ev) => {
                    try {
                        ev.stopPropagation();
                        ev.preventDefault();

                        // pega valores atuais/total dinamicamente
                        const current = (stat === 'pv' ? (typeof curPV !== 'undefined' ? curPV : 0)
                            : stat === 'mn' ? (typeof curMN !== 'undefined' ? curMN : 0)
                                : (typeof curSTA !== 'undefined' ? curSTA : 0));
                        const total = (stat === 'pv' ? (typeof totPV !== 'undefined' ? totPV : 0)
                            : stat === 'mn' ? (typeof totMN !== 'undefined' ? totMN : 0)
                                : (typeof totSTA !== 'undefined' ? totSTA : 0));

                        // SE não estiver cheio -> apenas incrementar (comportamento normal)
                        if (current < total) {
                            if (stat === 'pv') curPV = clamp(current + 1, 0, total);
                            else if (stat === 'mn') curMN = clamp(current + 1, 0, total);
                            else if (stat === 'sta') curSTA = clamp(current + 1, 0, total);
                        ...
                        // SENÃO (já está cheio): abrir modal para definir novo máximo
                        // chamamos async separado para manter fluxo simples
                        (async () => { await onPlusClickedFor(stat); })();

                    } catch (err) {
                        console.warn('Erro no plus handler custom:', err);
                    }
                }, { passive: false /* queremos poder preventDefault */ });
            });
        })();
     ...
     */
    // Versão simplificada e segura de revertLastLevel.
    // Reverte LVL em 1, retira último history entry e decrementa pontos_restantes.
    // NÃO altera PV/MN/STA.
    async function revertLastLevel() {
        if (!window.firebaseauth?.currentUser) return;
        const userDocRef = window.doc(window.firestoredb, window.firebaseauth.currentUser.uid);
        const userDocSnap = await window.getDoc(userDocRef);
        if (!userDocSnap.exists()) {
        ...
        // se não há histórico, ainda é possível rebaixar nível mas só ajustamos LVL/pontos_restantes
        const lastHist = Array.isArray(char.levelHistory) && char.levelHistory.length ? char.levelHistory.pop() : null;

        // decrementa nivel (mínimo 1)
        const oldLvl = Number(char.LVL ?? expLevel ?? 1);
        const newLvl = Math.max(1, oldLvl - 1);
        char.LVL = newLvl;

        // decrementa pontos_restantes apenas se > 0 (mantendo integridade)
        char.pontos_restantes = Math.max(0, Number(char.pontos_restantes ?? 0) - (lastHist ? Number(lastHist.pointsAdded ?? 1) : 1));

        // não alteramos PV/MN/STA (somente atualizamos a estrutura se necessário)
        char.EXP = char.EXP || {};
        char.EXP.atual = Number(currentEXP ?? 0);
        ...
        // atualiza local e UI
        charData = characters[idx];
        try { updateRemainingUI(); } catch (e) { /* silent */ }
        try {
            expLevel = Number(char.LVL ?? 1);
            const expLimit = expLimitForLevel(expLevel);
            applyBar('exp-bar-fill', 'exp-bar-text', currentEXP, expLimit, 'exp');
            const lvlEl = document.getElementById('exp-level');
            if (lvlEl) lvlEl.textContent = String(expLevel);
    ...
    (function enableLevelClickEdit() {
         ...
         */
        // Simplified batch level change: ajusta somente LVL e pontos_restantes (sem alterar PV/MN/STA).
        async function applyBatchLevelChange(newLevel) {
            ...
            const char = characters[idx];
            const oldLevel = Number(char.LVL ?? expLevel ?? 1);
            const target = Math.max(1, Number(newLevel || oldLevel));
            ...
            // se subindo: para cada nível ganho, add pontos_restantes +1 e registra history com zeros
            if (target > oldLevel) {
                char.levelHistory = Array.isArray(char.levelHistory) ? char.levelHistory : [];
                for (let lvl = oldLevel + 1; lvl <= target; lvl++) {
                    const prBefore = Number(char.pontos_restantes ?? 0);
                    char.pontos_restantes = prBefore + 1;
                    char.levelHistory.push({
                        ...
                        pointsAdded: 1,
                        profAdded: null,
                        attrsBefore: JSON.parse(JSON.stringify(char.atributos || {})),
                        pontosRestantesBefore: prBefore,
                        ts: Date.now()
                    });
                }
            } else {
                // se descendo: remover entradas de history correspondentes (se existirem) e reduzir pontos_restantes
                const levelsToRemove = oldLevel - target;
                char.levelHistory = Array.isArray(char.levelHistory) ? char.levelHistory : [];
                for (let i = 0; i < levelsToRemove; i++) {
                    const removed = char.levelHistory.pop();
                    // decrementa pontos_restantes conforme pointsAdded no history; se não existir, -1
                    const dec = removed ? Number(removed.pointsAdded ?? 1) : 1;
                    char.pontos_restantes = Math.max(0, Number(char.pontos_restantes ?? 0) - dec);
                }
            }

            // atualiza LVL/EXP.atual (EXP.atual deve ser tratado pelo chamador)
            char.LVL = target;
            char.EXP = char.EXP || {};
            char.EXP.atual = Number(currentEXP ?? 0);
...
/* === Talentos UI + modais === */
(function initTalentosFeature() {
    // evita múltiplas inicializações
    if (window.__talentosInitDone) return;
    window.__talentosInitDone = true;

    const battleTab = document.querySelector('.tab-content.tab-battle');
    if (!battleTab) return;

    const wrapper = document.getElementById('talentos-equip');
    if (!wrapper) return;

    // helpers Firestore
    const talentosColl = () => window.collection(window.firestoredb, 'talentos');

    // cria 4 slots (os 3 primeiros selecionáveis, o 4º trancado)
    function createSlots() {
        wrapper.innerHTML = '';
        for (let i = 0; i < 4; i++) {
            const slot = document.createElement('div');
            slot.className = 'talento-slot';
            slot.dataset.index = String(i);
            slot.tabIndex = 0;

            if (i < 3) {
                slot.classList.add('empty');
                slot.innerHTML = `<div class="talento-inner"><div class="talento-name">Clique para escolher</div></div>`;
            } else {
                slot.classList.add('locked');
                slot.innerHTML = `<div class="talento-inner"><div class="talento-name">Trancado</div><div style="font-size:0.85rem;opacity:0.9;">Clique para desbloquear</div></div>`;
            }

            slot.addEventListener('click', onSlotClick);
            slot.addEventListener('keydown', (e) => { if (e.key === 'Enter') slot.click(); });
            wrapper.appendChild(slot);
        }
    }

    // Carrega o estado atual de talentos do charData e preenche os slots
    async function renderSlotsFromChar() {
        const arr = Array.isArray(charData?.talentos) ? charData.talentos.slice() : [];
        const slots = Array.from(wrapper.querySelectorAll('.talento-slot'));
        // limpa todos
        slots.forEach(s => {
            s.classList.remove('filled');
            s.classList.remove('empty');
            // reset locked style only for index >=3
            const idx = Number(s.dataset.index);
            if (idx < 3) s.classList.add('empty');
            else s.classList.add('locked');
            s.innerHTML = `<div class="talento-inner"><div class="talento-name">${idx < 3 ? 'Clique para escolher' : 'Trancado'}</div></div>`;
        });

        // popular com os primeiros N talentos do array (ordem natural)
        for (let i = 0; i < Math.min(3, arr.length); i++) {
            ...
            try {
                ...
                const slot = wrapper.querySelector(`.talento-slot[data-index="${i}"]`);
                if (!slot) continue;
                if (data) {
                    slot.classList.remove('empty');
                    slot.classList.add('filled');
                    slot.innerHTML = `<div class="talento-inner"><div class="talento-name">${data.nome || '(sem nome)'}</div><div class="talento-effect">${data.efeito || ''}</div></div>`;
                    slot.dataset.uid = String(uid);
                } else {
                    // doc não encontrado — mostrar aviso e permitir remoção
                    slot.classList.remove('empty');
                    slot.classList.add('filled');
                    slot.innerHTML = `<div class="talento-inner"><div class="talento-name">(talento não encontrado)</div><div class="talento-effect">(uid: ${uid})</div></div>`;
                    slot.dataset.uid = String(uid);
                }
            } catch (err) {
                console.error('Erro carregando talento', uid, err);
            }
        }
    }
    ...
    async function onSlotClick(e) {
        ...
        if (slot.classList.contains('locked')) {
            ...
            openLockedConfirm(slot);
            return;
        }

        if (slot.classList.contains('filled')) {
            // abrir detalhe com opção de excluir
            openTalentDetailModal(slot.dataset.uid);
            return;
        }

        // slot vazio -> abrir modal de seleção
        openTalentosSelectionModal(idx);
    }

    /* ===== Modal de seleção de talentos (lista) ===== */
    const talentsBackdrop = document.getElementById('talentos-modal-backdrop');
    const talentsListEl = document.getElementById('talentos-list');
    const talentsCancelBtn = document.getElementById('talentos-modal-cancel');

    let activeSelectionSlotIndex = null;

    async function openTalentosSelectionModal(slotIndex) {
    ...
    talentsCancelBtn?.addEventListener('click', closeTalentosSelectionModal);
    talentsBackdrop?.addEventListener('click', (ev) => { if (ev.target === talentsBackdrop) closeTalentosSelectionModal(); });

    function closeTalentosSelectionModal() {
        if (!talentsBackdrop) return;
        talentsBackdrop.classList.remove('visible');
        setTimeout(() => { talentsBackdrop.style.display = 'none'; talentsListEl.innerHTML = ''; }, 260);
        activeSelectionSlotIndex = null;
    }

    // quando escolhe um talento na lista
    async function selectTalentForSlot(talUid) {
        if (activeSelectionSlotIndex === null) return;
        try {
            // ler array atual
            const arr = Array.isArray(charData?.talentos) ? charData.talentos.slice() : [];
            // se já existe esse uid em outro slot, remover para evitar duplicação
            const prevIdx = arr.indexOf(String(talUid));
            if (prevIdx !== -1) arr.splice(prevIdx, 1);

            // coloca na posição desejada (se índice maior que tamanho -> push)
            arr[activeSelectionSlotIndex] = String(talUid);

            // compactar (remover undefined vazios no tail) - opcional
            const compact = arr.filter(x => typeof x !== 'undefined' && x !== null && String(x) !== '');

            // salvar (usa saveCharacterField se existir, senão fallback robusto)
            if (typeof saveCharacterField === 'function') {
                await saveCharacterField('talentos', compact);
            } else if (typeof robustSaveCharacterField === 'function') {
                await robustSaveCharacterField('talentos', compact);
            } else {
                throw new Error('Função de salvar não encontrada.');
            }
            // atualiza charData local e UI
            charData.talentos = compact;
            await renderSlotsFromChar();
            closeTalentosSelectionModal();
        } catch (err) {
            console.error('Erro ao salvar talento no personagem', err);
            window.alert('Erro ao salvar talento. Veja console.');
        }
    }

    /* ===== Modal de detalhe do talento (abrir a partir do slot preenchido) ===== */
    ...
    detailDeleteBtn?.addEventListener('click', async () => {
        ...
        try {
            ...
            for (let i = arr.length - 1; i >= 0; i--) {
                if (String(arr[i]) === String(currentDetailUid)) { arr.splice(i, 1); changed = true; }
            }
            if (!changed) { window.alert('Talento não encontrado no personagem.'); closeTalentDetailModal(); return; }

            // salvar
            if (typeof saveCharacterField === 'function') {
                await saveCharacterField('talentos', arr);
            } else if (typeof robustSaveCharacterField === 'function') {
                await robustSaveCharacterField('talentos', arr);
            }

            charData.talentos = arr;
            await renderSlotsFromChar();
            closeTalentDetailModal();
        } catch (err) {
            console.error('Erro ao remover talento', err);
            window.alert('Erro ao remover talento. Veja console.');
        }
    });
    ...
    async function openLockedConfirm(slotEl) {
        ...
        // novo slot vazio (substituto)
        const newSlot = document.createElement('div');
        newSlot.className = 'talento-slot empty';
        newSlot.dataset.index = slotEl.dataset.index;
        newSlot.tabIndex = 0;
        newSlot.innerHTML = `<div class="talento-inner"><div class="talento-name">Clique para escolher</div></div>`;
        newSlot.addEventListener('click', onSlotClick);
        newSlot.addEventListener('keydown', (e) => { if (e.key === 'Enter') newSlot.click(); });

        // atual slot trancado -> mover para final e atualizar index
        const oldLocked = slotEl;
        const maxIndex = wrapper.querySelectorAll('.talento-slot').length - 1;
        // replace in place
        wrapper.replaceChild(newSlot, oldLocked);

        // criar um novo locked slot no final
        const lockedSlot = document.createElement('div');
        lockedSlot.className = 'talento-slot locked';
        lockedSlot.dataset.index = String(maxIndex);
        lockedSlot.tabIndex = 0;
        lockedSlot.innerHTML = `<div class="talento-inner"><div class="talento-name">Trancado</div><div style="font-size:0.85rem;opacity:0.9;">Clique para desbloquear</div></div>`;
        lockedSlot.addEventListener('click', onSlotClick);
        lockedSlot.addEventListener('keydown', (e) => { if (e.key === 'Enter') lockedSlot.click(); });
        wrapper.appendChild(lockedSlot);

        // Reindexa todos os slots dataset.index (para manter ordem)
        Array.from(wrapper.querySelectorAll('.talento-slot')).forEach((s, i) => s.dataset.index = String(i));
    }

    // inicial
    createSlots();
    // espera que charData esteja pronto (já é carregado no arquivo principal)
    (async () => {
        try {
            // se charData estiver indefinido, aguarda um pouco (várias partes do seu arquivo usam onAuthStateChanged)
            const waitForChar = () => new Promise(res => {
                const tries = 20;
                let t = 0;
                const iv = setInterval(() => {
                    if (typeof charData !== 'undefined' && charData) { clearInterval(iv); return res(); }
                    t++;
                    if (t > tries) { clearInterval(iv); return res(); }
                }, 180);
            });
            await waitForChar();
        } catch (e) {/* ignore */ }
        try { await renderSlotsFromChar(); } catch (e) { console.warn('renderSlotsFromChar erro:', e); }
    })();

    // expõe função pública para forçar refresh (usado por outras partes do código)
