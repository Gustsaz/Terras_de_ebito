...
/* === TALENTOS: list / edit / create / delete === */
(function () {
    const TALENTOS_COLLECTION = 'talentos';

    const openBtn = document.getElementById('open-add-talento');
    const panel = document.getElementById('add-talento-panel');
    const closeBtn = panel?.querySelector('.close-add-talento');
    const form = document.getElementById('add-talento-form');

    const talentosListEl = document.getElementById('talentos-list');
    const novoTalBtn = document.getElementById('novo-talento-btn');
    const refreshTalBtn = document.getElementById('refresh-talentos-btn');
    const cancelBtn = document.getElementById('cancel-add-talento');
    ...
    async function loadTalentos() {
        try {
            if (!talentosListEl) return;
            talentosListEl.innerHTML = '<p>Carregando...</p>';
            const fns = await resolveFirestoreFunctions();
            const colRef = fns.collection(window.firestoredb, TALENTOS_COLLECTION);
            const snap = await fns.getDocs(colRef);
            talentosListEl.innerHTML = '';
            if (!snap || snap.size === 0) {
                talentosListEl.innerHTML = '<p>Nenhum talento cadastrado.</p>';
                return;
            }
            snap.forEach(doc => {
                const data = doc.data() || {};
                ...
                card.innerHTML = `
          <h4>${escapeHtml(data.nome || 'Sem nome')}</h4>
          <p><strong>Requisito:</strong> ${escapeHtml(data.requisito || '-')}</p>
          <p>${escapeHtml((data.efeito || '').slice(0, 140))}${(data.efeito && data.efeito.length > 140) ? '...' : ''}</p>
          <div class="card-actions">
            <button data-edit-id="${id}" class="edit-talento-btn">Editar</button>
            <button data-delete-id="${id}" class="del-talento-btn">Excluir</button>
          </div>
        `;
                talentosListEl.appendChild(card);
            });

            // attach handlers
            talentosListEl.querySelectorAll('.edit-talento-btn').forEach(btn => {
                btn.addEventListener('click', async () => {
                    const id = btn.dataset.editId;
                    try {
                        const fns = await resolveFirestoreFunctions();
                        const docRef = fns.doc(window.firestoredb, TALENTOS_COLLECTION, id);
                        const snap = await fns.getDoc(docRef);
                        if (!snap.exists()) return alert('Documento não encontrado.');
                        const data = snap.data() || {};
                        document.getElementById('talento_nome').value = data.nome || '';
                        document.getElementById('talento_requisito').value = data.requisito || '';
                        document.getElementById('talento_efeito').value = data.efeito || '';
                        form.setAttribute('data-doc-id', id);
                        panel.style.display = 'block';
                    } catch (err) {
                        console.error('Erro ao abrir talento para editar:', err);
                        alert('Erro ao carregar talento (veja console).');
                    }
                });
            });

            talentosListEl.querySelectorAll('.del-talento-btn').forEach(btn => {
                btn.addEventListener('click', async () => {
                    const id = btn.dataset.deleteId;
                    if (!confirm('Confirma excluir este talento?')) return;
                    try {
                        const fns = await resolveFirestoreFunctions();
                        const docRef = fns.doc(window.firestoredb, TALENTOS_COLLECTION, id);
                        if (typeof fns.deleteDoc === 'function') {
                        ...
                        console.error('Erro excluindo talento:', err);
                        alert('Erro ao excluir (veja console).');
                    }
                });
            });
            ...
            if (talentosListEl) talentosListEl.innerHTML = '<p>Erro ao carregar talentos.</p>';
        }
    }
    ...
    // eventos de UI
    novoTalBtn?.addEventListener('click', openFormForNew);
    refreshTalBtn?.addEventListener('click', loadTalentos);
    closeBtn?.addEventListener('click', () => panel.style.display = 'none');
    cancelBtn?.addEventListener('click', () => panel.style.display = 'none');
    if (openBtn) openBtn.addEventListener('click', openFormForNew);

    // submit: criar/atualizar
    form.addEventListener('submit', async (e) => {
        e.preventDefault();
        try {
            const nome = document.getElementById('talento_nome').value.trim();
            const requisito = document.getElementById('talento_requisito').value.trim();
            const efeito = document.getElementById('talento_efeito').value.trim();

            if (!nome) { alert('Preencha ao menos o nome.'); return; }
            const payload = { nome, requisito, efeito };

            const docId = form.getAttribute('data-doc-id');
            const fns = await resolveFirestoreFunctions();
            if (docId) {
                const docRef = fns.doc(window.firestoredb, TALENTOS_COLLECTION, docId);
                if (typeof fns.setDoc === 'function') await fns.setDoc(docRef, payload, { merge: true });
                else if (typeof fns.updateDoc === 'function') await fns.updateDoc(docRef, payload);
                else await fns.addDoc(fns.collection(window.firestoredb, TALENTOS_COLLECTION), payload);
                alert('Talento atualizado.');
            } else {
                await fns.addDoc(fns.collection(window.firestoredb, TALENTOS_COLLECTION), payload);
                alert('Talento criado.');
            }
    ...
    (async () => {
        try {
            ...
            console.warn('Talentos: Firestore pode não estar pronto ainda:', e);
        }
    })();
